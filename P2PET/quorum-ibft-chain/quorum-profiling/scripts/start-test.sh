...
JMXFILE=${homeDir}/jmeter-test/${jmeter_test_profile}.jmx
...
tpsDockerImg="quorumengineering/tpsmonitor:v1"
jmeterDockerImg="quorumengineering/jmeter:5.2.1"
network_name="$(basename $PWD)_monitoring"

echo "start jmeter profile ${jmeter_test_profile}.."
docker run -d --network $network_name -v ${homeDir}/scripts/jmeter:/stresstest \
 --name jmeter  ${jmeterDockerImg} -n -t /stresstest/${jmeter_test_profile}.jmx -q /stresstest/network.properties -j /stresstest/jmeter.log -l /stresstest/results.jtl
echo "jmeter test profile ${jmeter_test_profile} started"


echo "start tps monitor..."
docker run -d --network $network_name -v ${homeDir}/scripts/tpsmon:/tpsmon -p 7777:7777 -p 2112:2112 --name tps-monitor ${tpsDockerImg}  --httpendpoint ${quorumEndpoint} --consensus=${consensus} --report /tpsmon/tps-report.csv --prometheusport 2112 --port 7777 --influxdb --influxdb.endpoint "http://influxdb:8086" --influxdb.token "telegraf:test123"
