...
export const getBiddingResults = async () => {
  const { data } = await api.get("/bidding-results");
  return data;
};

export const getRemainingTimeInPhase = async () => {
  const { data } = await api.get("/remaining_time_in_phase");
  return data;
};
